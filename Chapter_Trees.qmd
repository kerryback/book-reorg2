...
This motivates defining two numbers $p_u$ and $p_d$:
...
Notice that $p_u>0$, $p_d>0$, and $p_u+p_d=1$.  This makes it possible to view $p_u$ and $p_d$ as artificial probabilities.  We make no claim that they represent anyone's beliefs, but they have the mathematical properties of probabilities.  They are universally called the `risk-neutral probabilities.'  The motivation for this terminology is that, as the previous equation shows, option values are discounted `expected values' using these probabilities to compute the expectation.  This is how options would be valued if investors were risk neutral.  We **do not** assume that investors are risk neutral; nevertheless, we can compute option values **as if** investors were risk-neutral, with beliefs represented by the risk-neutral probabilities.

Rather than first calculating the state prices, we can calculate the risk-neutral probabilities directly as
