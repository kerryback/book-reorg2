...
Options have a finite lifetime, with each contract specifying an expiration date. Most exchange-traded options can be exercised at any time prior to expiration (**American options**), while others can only be exercised at the expiration date (**European options**). Despite their names, both types trade on exchanges worldwide.  Options on individual stocks are ordinarily American, while options of stock indices are typically European.
...
Options are actively traded on organized exchanges and also arranged as private contracts (called over-the-counter or OTC). Exchange-traded options offer standardized contracts, while OTC options can be customized but involve counterparty risk.
...
- *Volume*: the number of contracts traded during the day.  The size of a contract (for example, the number of shares for stock options) is determined by the exchange on which the contract is listed.
