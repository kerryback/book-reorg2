...
$$ {#eq-exponential1}
for constants $\mu$ and $\sigma$, where $B$ is a Brownian motion.  For each time $t$, the random variable $S_t$ in @eq-exponential1 is a lognormal random variable. Ito's formula for exponentials of Ito processes implies^[Setting $X_t = \mu t - \sigma^2 t/2 + \sigma B_t$, we have S_t = \mathrm{e}^{X_t}, so $\mathrm d S/S = \mathrm d X + \frac{1}{2}(\mathrm d X)^2 = \mu \mathrm d t - \sigma^2/2 \mathrm d t + \sigma \mathrm d B + \frac{1}{2}(\mathrm{d} X)^2 = \mu\mathrm{d}   t+\sigma\mathrm{d}   B$.]
$$
...
$$ 
S_t > K  \quad\Longleftrightarrow\quad \log S_0 + \left(\mu + \frac{1}{2}\sigma^2 \right) t+ \sigma B_t > \log K - \log S_0-\left(\mu - \frac{1}{2}\sigma^2 \right) t\,.
$$
