...
Another useful representation of pricing in a finite-state world is obtained by dividing each state price by the probability of the state.  Set $m_j = \pi_j/\text{prob}_j$, where $\text{prob}_j$ denotes the probability of state $j$ (the actual probability, not the risk-neutral probability).  Then,
$$
V = \sum_{j=1}^J \text{prob}_jm_jV_j\,.
$$ {#eq-arbpricing-sdf0}
This is the expectation (mean) of the random variable with values $m_jV_j$.  In general, we denote expectation by the symbol $\mathbb{E}$.
