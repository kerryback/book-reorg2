...
3. The pricing operator obeys time value of money: $1=\sum_{j=1}^J \pi_j \mathrm{e}^{rt}$, where $r$ is the continuously compounded yield to maturity of a bond maturing at $t$. This is the present value rule: the present value of the future value $\mathrm{e}^{rt}$ is one or, equivalently, the present value of a dollar paid at $t$ is $\mathrm{e}^{-rt}$.
...
We saw these principles and  calculated state prices in the binomial model ($J=2$ and two assets) in @sec-c:trees.  To calculate state prices for general finite $J$, we need to find $J$ state prices $\pi_1, \ldots, \pi_J$. If we have $J$ traded assets (including the risk-free asset) with linearly independent payoffs, then we can solve:

$$\begin{bmatrix} 
\mathrm{e}^{rt} & \mathrm{e}^{rt} & \cdots & \mathrm{e}^{rt} \\
...
With state prices determined, we can calculate the value $V$ of any derivative security with payoffs $V_j$ as:

$$
V = \sum_{j=1}^J \pi_j V_j\,.
$$ {#eq-arbpricing-stateprices}

The risk-neutral probabilities are defined as in @sec-c:trees: $p_j = \pi_j \mathrm{e}^{rt}$, giving

$$
V = \mathrm{e}^{-rt} \sum_{j=1}^J p_j V_j = \mathrm{e}^{-rt} \mathbb{E}^p[V_t]\,.
$$ {#eq-arbpricing-rnp0}

We call the collection of risk-neutral probabilities $(p_1, \ldots, p_J)$ the risk-neutral probability measure or, for brevity, the risk-neutral measure.  We also use the term 'risk-neutral measure' to describe the risk-neutral probability distribution when there is a continuum of states.

Another useful representation of pricing in a finite-state world is obtained by dividing each state price by the probability of the state.  Set $m_j = \pi_j/\text{prob}_j$, where $\text{prob}_j$ denotes the probability of state $j$ (the actual probability, not the risk-neutral probability).  @eq-arbpricing-stateprices can be written as 
$$
V = \sum_{j=1}^J \text{prob}_jm_jV_j\,.
$$ {#eq-arbpricing-sdf0}
Thus, the date-0 value $V$ is the expectation (mean) of the random variable with values $m_jV_j$.  In general, we denote expectation by the symbol $\mathbb{E}$.

The random variable $m$ in the previous paragraph is called the **stochastic discount factor** or SDF for short.  

## Continuum of States

We now extend to models where the stock price can take a continuum of values. The formulas for an asset value in the finite-state model generalize to models with a continuum of states.  We begin with the formula in terms of an SDF.

The SDF formula @eq-arbpricing-sdf0 generalizes to models with a continuum of states, even though individual states have zero probability, making the division $\pi_j/\text{prob}_j$ impossible.  In such models, the SDf is sometimes called a **state price density**, the term density having a similar meaning here as it does in the 'probability density function' of a random variable that one encounters in statistics.  The fundamental principle that underlies option valuation is:

::: Principle
If there are no arbitrage opportunities,^[The definition of 'arbitrary opportunity' that is required for this statement to be correct is somewhat complex.  See [@back2010] for a survey of work on this question and [@LoewensteinWillard2000] and [@HestonLoewensteinWillard2007].] then there exists for each date $t$ a strictly positive random variable $m_t$, called a stochastic discount factor, such that the date-0 value of any dividend-reinvested asset with price $P$ is:
...
From @eq-arbpricing-sdf1, we can construct a risk-neutral measure and apply 'risk-neutral pricing' as in @eq-arbpricing-rnp0.
First, note that the pricing formula @eq-arbpricing-sdf1 applied to the risk-free asset gives:

$$
1 = \mathbb{E}[m_t \mathrm{e}^{rt}]\,.
$$ {#eq-arbpricing-mean1}

This means that $m_t \mathrm{e}^{rt}$ has expectation 1. We can define a new probability measure by:

$$\mathbb{E}^R[Z] = \mathbb{E}[m_t \mathrm{e}^{rt} Z]$$

for any random variable $Z$. Because $\mathbb{E}[m_t\mathrm{e}^{rt}]=1$, \mathbb{E}^R[1]=1$, which is required for a probability measure.  Under this probability measure:

$$
P_0 = \mathrm{e}^{-rt} \mathbb{E}^R[P_t]\,.
$$ {#eq-}
...
$$\frac{X(0)}{R(0)} = \sum_{s=1}^S p_s \frac{X_s}{\mathrm{e}^{rt}} = \mathbb{E}^R\left[\frac{X}{R_t}\right]$$

where $R(0) = 1$ and $R_t = \mathrm{e}^{rt}$. The ratio of any asset price to the risk-free asset price is a martingale under the risk-neutral measure.
...
$$Y_s = \mathrm{e}^{-r(t-s)} \mathbb{E}^R_s[Y_t]$$
...
- Risk-free bond: pays $\mathrm{e}^{0.05}$ in all states  
...
a) Find the no-arbitrage bounds: show that $S_d < S_0 \mathrm{e}^r < S_u$ must hold
...
a) $\mathrm{e}^{-qt}S_t$ is a martingale under the risk-neutral measure
b) $S_t/R_t$ is a martingale under the risk-neutral measure, where $R_t = \mathrm{e}^{rt}$
c) If $V_t$ is the value of any derivative, then $\mathrm{e}^{-rt}V_t$ is a martingale under the risk-neutral measure
:::
...
a) Verify that $\mathbb{E}^R[S_T \mathrm{e}^{-qT}] \approx S_0$ (martingale property)
b) Verify that $\mathbb{E}^R[\mathrm{e}^{-rT}S_T] \approx S_0 \mathrm{e}^{-qT}$ (discounted expectation)
...
a) Risk-neutral measure: $\mathbb{E}^R[\mathrm{e}^{-rT} \mathbf{1}_{\{S_T \geq K\}}]$
...
b) Prove that $\mathbb{E}[mR] = 1$ where $R = \mathrm{e}^{rT}$ is the gross risk-free return
...
a) Using the fundamental pricing formula, show that this can be priced as $\mathbb{E}^R[\mathrm{e}^{-rT}\max(S_T^{(1)}, S_T^{(2)}, K)]$
