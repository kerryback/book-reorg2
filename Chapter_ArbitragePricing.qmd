...
We calculated state prices in the binomial model ($J=2$ and two assets) in @sec-c:trees.  For general finite $J$, we need to find $J$ state prices $\pi_1, \ldots, \pi_J$. If we have $J$ traded assets (including the risk-free asset) with linearly independent payoffs, then we can solve:
...
The risk-neutral probabilities are defined as in @sec-c:trees: $p_j = \pi_j e^{rt}$, giving

$$V = e^{-rt} \sum_{j=1}^J p_j V_j = e^{-rt} \mathbb{E}^p[V_t]\,.$$

We call the collection of risk-neutral probabilities $(p_1, \ldots, p_J)$ the risk-neutral probability measure or, for brevity, the risk-neutral measure.  We use the same terminology to describe the risk-neutral probability distribution when there is a continuum of states.

Another useful representation of pricing is obtained by dividing each state price by the probability of the state.  Set $m_j = \pi_j/\text{prob}_j$, where $\text{prob}_j$ denotes the probability of state $j$ (the actual probability, not the risk-neutral probability).  Then,
...
The random variable $m$ in the previous paragraph is called the **stochastic discount factor** or SDF for short.  This representation of an asset value generalizes to models with a continuum of states, even though individual states have zero probability, making the division $\pi_j/\text{prob}_j$ impossible.  In such models, the SDf is sometimes called a **state price density**, the term density having a similar meaning here as it does in the 'probability density function' of a random variable that one encounters in statistics.
...
We can construct the risk-neutral probability measure from the SDF $m$.  First, note that the pricing formula applied to the risk-free asset gives:
